                    ...
                    <div className="bg-gray-800/40 border border-gray-700 rounded-xl p-2 sticky top-24">
                        <div className="px-4 py-3 mb-2 border-b border-gray-700/50">
                            <h1 className="text-sm font-bold text-gray-400 tracking-widest uppercase flex items-center gap-2">
                                <i className="fa-solid fa-shield-halved text-blue-500"></i> 系统模块
                            </h1>
                        </div>
                                ...
                                <div className="bg-gray-800/20 border border-gray-800 rounded-xl overflow-x-auto">
                                    <table className="w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y divide-gray-800/30 text-gray-300">
                                            {isLoading ? (
                                                <tr><td colSpan="4" className="px-6 py-12 text-center text-gray-500 animate-pulse">读取核心档案中...</td></tr>
                                                    ...
                                                    <tr key={user.username} className="hover:bg-white/5 transition-colors group">
                                                        ...
                                                        <td className="px-6 py-4 text-right">
                                                            <div className="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                                                ...
                                                                {user.status === 'banned' ? (
                                                                    <button onClick={() => handleUserAction(user.username, 'unban')} className="w-7 h-7 flex items-center justify-center rounded bg-green-900/20 text-green-500 hover:bg-green-900/40 border border-green-700/20" title="解封"><i className="fa-solid fa-unlock text-xs"></i></button>
                                                                ) : (
                                                                    <button onClick={() => handleUserAction(user.username, 'ban')} className="w-7 h-7 flex items-center justify-center rounded bg-yellow-900/20 text-yellow-500 hover:bg-yellow-900/40 border border-yellow-700/20" title="封禁"><i className="fa-solid fa-ban text-xs"></i></button>
                                                                )}
                                                                <button onClick={() => handleUserAction(user.username, 'delete')} className="w-7 h-7 flex items-center justify-center rounded bg-red-900/20 text-red-500 hover:bg-red-900/40 border border-red-700/20" title="销毁档案"><i className="fa-solid fa-trash-can text-xs"></i></button>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                ))
                        ...
                        {activeTab === 'quarantine' && (
                            <div className="space-y-8">
                                <div className="p-5 bg-orange-900/10 border border-orange-800/30 rounded-xl flex gap-4 items-start shadow-inner">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-orange-300">数据源隔离协议 (Quarantine Net)</h4>
                        ...
                        {activeTab === 'logs' && (
                            <div className="bg-gray-800/20 border border-gray-800 rounded-xl overflow-hidden shadow-inner">
                                <ul className="divide-y divide-gray-800/30">
                                    {logs.length === 0 ? (
                                        <li className="p-12 text-center text-gray-600 italic">尚未产生可审计的记录</li>
                                    ) : (
                                        logs.map((log, index) => {
                                            ...
                                            return (
                                                <li key={index} className="p-4 hover:bg-white/5 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <div className="text-sm font-medium text-gray-200">
                                                                ...
                                                                <span className="opacity-60">{l.action === 'buy' ? '执行买入' : l.action === 'sell' ? '执行卖出' : '执行指令'}</span>
                                                    ...
                                                    <div className="text-right">
                                                        {l.amount && <div className="text-sm font-black text-white font-mono">{l.amount} <span className="text-[10px] text-gray-600 font-normal">SHARES</span></div>}
                                                        {l.price && <div className="text-[10px] text-gray-500 font-mono">EST. ¥{l.price.toFixed(2)}</div>}
                            ...
                            <div className="max-w-3xl mx-auto py-12">
                                <div className="bg-gray-800/20 border border-gray-800 p-8 rounded-2xl shadow-xl">
                                    <div className="flex items-center gap-4 mb-8">
                                        ...
                                        <div>
                                            <h3 className="font-bold text-white text-lg leading-tight">全站横幅广播</h3>
                                            <p className="text-xs text-gray-500 mt-1 uppercase tracking-widest">Global Broadcast System</p>
                                        </div>
                                    </div>
                                    <textarea 
                                        value={broadcastMsg}
                                    ...
                                    <div className="mt-6 flex justify-between items-center">
                                        <span className="text-[10px] text-gray-600 font-mono italic">字符限制: 500 (建议简短)</span>
                                        <div className="flex gap-3">
                                            <button onClick={() => setBroadcastMsg('')} className="px-5 py-2 text-gray-500 hover:text-white transition-colors text-xs font-bold uppercase tracking-widest">Clear</button>
                                            <button onClick={saveBroadcast} className="px-8 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-black transition-all shadow-lg text-xs uppercase tracking-widest">Deploy Message</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-8">
                                <div className="bg-gray-800/20 border border-gray-800 p-8 rounded-2xl flex flex-col justify-between">
                                    ...
                                    <div className="mt-10 space-y-4">
                                        ...
                                        <button onClick={() => executeMacro('airdrop')} className="w-full py-3 bg-green-700 hover:bg-green-600 text-white rounded-xl font-black transition-all shadow-lg text-xs uppercase tracking-widest">Execute Airdrop</button>
                                    </div>
                                </div>
                                ...
                                <div className="bg-gray-800/20 border border-gray-800 p-8 rounded-2xl flex flex-col justify-between">
                                    <div>
                                        ...
                                        <h3 className="font-bold text-white text-lg">全局余额征税</h3>
                                        <p className="text-xs text-gray-500 mt-2 leading-relaxed">按比例对全站用户当前余额进行回收。通常用于抑制通货膨胀或大型系统维护前夕的数据调配。</p>
                                    </div>
                                    <div className="mt-10 space-y-4">
                                        <div className="bg-gray-950 border border-gray-800 rounded-lg px-4 py-3 flex justify-between items-center shadow-inner">
                                            <span className="text-xs text-gray-500 font-bold uppercase tracking-widest">Tax Rate (%)</span>
                                        ...
                                        <button onClick={() => executeMacro('tax')} className="w-full py-3 bg-red-700 hover:bg-red-600 text-white rounded-xl font-black transition-all shadow-lg text-xs uppercase tracking-widest">Execute Taxation</button>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'settings' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="bg-gray-800/20 border border-gray-800 rounded-2xl p-8 flex flex-col">
                                    <div className="flex items-center gap-4 mb-8">
                                        <div className="w-10 h-10 bg-teal-900/20 rounded-lg flex items-center justify-center text-teal-400 border border-teal-800/30 font-black">B</div>
                                        <h3 className="font-bold text-white text-sm uppercase tracking-widest">Tag Bingo 规则配置</h3>
                                    </div>
                                    <div className="space-y-6 flex-1">
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">候选标签池</label>
                                            <textarea value={bingoTagsInput} onChange={e => setBingoTagsInput(e.target.value)} className="w-full h-24 bg-gray-950 border border-gray-800 rounded-xl p-4 text-xs text-white focus:border-teal-500 transition-all shadow-inner custom-scrollbar" placeholder="原创, scp, tale..." />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">扫描价格 (¥)</label>
                                    ...
                                    <button onClick={saveBingoSettings} className="mt-8 w-full py-3 bg-teal-700/80 hover:bg-teal-600 text-white rounded-xl font-black transition-all shadow-lg text-xs uppercase tracking-widest">Save Config</button>
                                </div>

                                <div className="bg-gray-800/20 border border-gray-800 rounded-2xl p-8 flex flex-col">
                                    <div className="flex items-center gap-4 mb-8">
                                        <div className="w-10 h-10 bg-orange-900/20 rounded-lg flex items-center justify-center text-orange-400 border border-orange-800/30 font-black">H</div>
                                        <h3 className="font-bold text-white text-sm uppercase tracking-widest">Bounty Hunter 配置</h3>
                                    </div>
                                    <div className="space-y-4 flex-1">
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">目标标签</label>
                                            <textarea value={bountyTagsInput} onChange={e => setBountyTagsInput(e.target.value)} className="w-full h-20 bg-gray-950 border border-gray-800 rounded-xl p-4 text-xs text-white focus:border-orange-500 transition-all shadow-inner custom-scrollbar" placeholder="原创, 精品, scp..." />
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Max Rating</label>
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Base Reward (¥)</label>
                                    ...
                                    <button onClick={saveBountySettings} className="mt-8 w-full py-3 bg-orange-700/80 hover:bg-orange-600 text-white rounded-xl font-black transition-all shadow-lg text-xs uppercase tracking-widest">Apply Reward Rule</button>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="max-w-4xl mx-auto">
                                <div className="bg-gray-950 border border-red-900/30 rounded-2xl shadow-2xl overflow-hidden">
                                    <div className="bg-red-900/20 px-6 py-3 flex items-center justify-between border-b border-red-900/20">
                                        ...
                                        <span className="text-[10px] text-red-400 font-black tracking-[0.2em] uppercase">Kernel Access / Redis Terminal</span>
                                    </div>
                                    <div className="p-8 space-y-6">
                                        <div className="flex flex-col md:flex-row gap-4">
                                            <div className="flex-1 relative group">
                                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-red-900 font-mono text-xs uppercase font-black transition-colors group-focus-within:text-red-500">KEY &gt;</span>
                                            ...
                                            <button onClick={() => queryRedis('get')} className="px-8 py-3 bg-gray-900 hover:bg-gray-800 text-white font-black rounded-xl transition-all border border-gray-800 text-xs uppercase tracking-widest">Fetch Data</button>
                                        </div>
                                        <textarea value={redisValue} onChange={e => setRedisValue(e.target.value)} className="w-full h-80 bg-black border border-gray-900 rounded-xl p-5 text-green-500 font-mono text-xs focus:outline-none focus:border-red-900 shadow-inner custom-scrollbar" placeholder="RAW DATA BUFFER..." />
                                        <div className="flex items-center gap-4">
                    ...
                    <div className="bg-gray-950 border border-gray-800 rounded-2xl w-full max-w-xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-8 overflow-y-auto custom-scrollbar flex-1 space-y-8">
                            <div className="p-6 bg-blue-600/5 rounded-2xl border border-blue-500/10 flex justify-between items-center">
                                <span className="text-xs text-gray-500 font-bold uppercase tracking-widest">Account Balance</span>
                            ...
                            <div className="space-y-3">
                                <h4 className="text-[10px] font-bold text-gray-600 uppercase tracking-[0.2em] mb-4">Portfolio Details</h4>
                                {Object.entries(inspectData).filter(([k]) => k !== 'balance').map(([key, value]) => {
                                    ...
                                    return (
                                        <div key={key} className="flex justify-between items-center bg-gray-900/30 border border-gray-800 p-4 rounded-xl">
                                            ...
                                            <div className="text-right">
                                                <span className={`font-mono font-black text-sm ${shares > 0 ? 'text-blue-500' : 'text-orange-500'}`}>{shares > 0 ? 'LONG' : 'SHORT'} {Math.abs(shares)} </span>
                                                <div className="text-[10px] text-gray-600 font-mono mt-0.5">AVG. ¥{Number(cost).toFixed(2)}</div>
                                            </div>
                                        </div>
                                    );
                                })}
                            ...
                            <div className="pt-8 border-t border-gray-800">
                                <h4 className="text-[10px] font-bold text-orange-600 uppercase tracking-[0.2em] mb-4">Direct Intervention</h4>
                                <div className="bg-orange-950/10 p-6 rounded-2xl border border-orange-900/20 space-y-4 shadow-inner">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="text-[10px] text-gray-600 font-bold uppercase mb-2 block tracking-widest">Adjust Amount</label>
                                            <input type="number" value={adjustAmount} onChange={e => setAdjustAmount(e.target.value)} placeholder="± 1000" className="w-full bg-black border border-gray-800 rounded-lg p-3 text-white font-mono text-sm focus:border-orange-900 outline-none"/>
                                        </div>
                                        <div>
                                            <label className="text-[10px] text-gray-600 font-bold uppercase mb-2 block tracking-widest">Audit Note</label>
                                            <input type="text" value={adjustNote} onChange={e => setAdjustNote(e.target.value)} placeholder="Reason for change" className="w-full bg-black border border-gray-800 rounded-lg p-3 text-white text-xs focus:border-orange-900 outline-none"/>
                                        </div>
                                    </div>
                                    <button onClick={handleAdjustBalance} disabled={isAdjusting} className="w-full py-3.5 bg-orange-900/50 hover:bg-orange-800 disabled:bg-gray-900 text-orange-500 font-black rounded-xl transition-all text-xs uppercase tracking-widest border border-orange-800/30">
                                        {isAdjusting ? 'Processing Transaction...' : 'Execute Balance Adjustment'}
                                    </button>
                                </div>
                            </div>
                        </div>
